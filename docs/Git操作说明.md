...
本说明主要供各组组长使用。普通组员只需按规范完成代码，并将文件交给组长；由组长统一提交到 GitHub。

## 基本规范

- 只修改本组负责的章节目录，不修改其他章节和仓库根目录。
- 不上传医学原始数据、患者信息、医学影像、模型文件或大型运行结果。
- 不在代码中写入 API Key、密码、Token 等敏感信息；本地密钥放在 `.env` 中，但不要提交 `.env`。
...
将组员提交的文件整理到 `chapters/chapter-01/` 中，然后检查改动：
